...
class Application():
    ...
    def user_account(self):
        ...
        return template('app/views/html/user_account.html', user=user)
    ...
    def edit_user(self):
        ...
        if request.method == 'POST':
            ...
            if new_email != user_to_edit.email and any(u.email == new_email for u in users):
                return template('app/views/html/user_account.html', user=user_to_edit, error="Este email já está em uso.")

            user_to_edit.email = new_email
            user_to_edit.username = new_username

            if new_password:
                user_to_edit.password_hash = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt())
            ...
            if upload and upload.filename:
                ...
                if user_to_edit.profile_image and 'default.png' not in user_to_edit.profile_image:
                    old_image_path = os.path.join(os.path.dirname(__file__), '..', user_to_edit.profile_image.lstrip('/'))
                    if os.path.exists(old_image_path):
                        os.remove(old_image_path)
                
                upload.save(save_path)
                user_to_edit.profile_image = f"/static/img/profile/{unique_filename}"

            self._save_users(users)
            response.set_cookie("user_email", user_to_edit.email, secret='uma-chave-secreta-muito-forte')
            return template('app/views/html/user_account.html', user=user_to_edit, success="Dados atualizados com sucesso!")
    ...
    def delete_user(self):
        ...
        users = self._load_users()
        user_to_delete = next((u for u in users if u.email == user_email), None)

        if user_to_delete:
            if user_to_delete.profile_image and 'default.png' not in user_to_delete.profile_image:
                image_path = os.path.join(os.path.dirname(__file__), '..', user_to_delete.profile_image.lstrip('/'))
                if os.path.exists(image_path):
                    os.remove(image_path)

        users_to_keep = [user for user in users if user.email != user_email]
        self._save_users(users_to_keep)
        response.delete_cookie("user_email")
        redirect('/GAMEVERSE')